...
void CTFWeaponBase::ApplyOnHitAttributes( CBaseEntity *pVictimBaseEntity, CTFPlayer *pAttacker, const CTakeDamageInfo &info )
{
	...
	*/
	...
	if ( pVictim )
	{
		...
		//apply half crit debuff
		int MarkHCrit = 0;
		CALL_ATTRIB_HOOK_INT(MarkHCrit,Damned);
		if(MarkHCrit){
			if ( pAttacker->m_pDam != NULL && pAttacker->m_pDam->m_Shared.InCond( TF_COND_HCRIT )){
				pAttacker->m_pDam->m_Shared.RemoveCond( TF_COND_HCRIT );
			}
			float HCritDuration = 16.0f;
			pVictim->m_Shared.AddCond( TF_COND_HCRIT, HCritDuration, pAttacker );
			pAttacker->m_pDam = pVictim;
		}
